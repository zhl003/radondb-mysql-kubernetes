...
func (o operator) LoadDump(ctx context.Context, dir FsInfo) error {
	args := []string{
		...
		"util",
		"load-dump",
		"--dump-dir", filepath.Join(dir.FsPath, dir.FsName),
...
func (o operator) LoadBinlog(ctx context.Context, binlogDir FsInfo, restorePoint time.Time) error {
	dirents, err := os.ReadDir(binlogDir.FsPath)
	if err != nil {
		return err
	}
	var binlogs []string
	...
	for i, n := range binlogs {
		binlogFiles[i] = filepath.Join(binlogDir.FsPath, n)
	}
	...
	tmpDir := filepath.Join(binlogDir.FsPath, "tmp")
