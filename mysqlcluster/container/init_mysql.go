...
func (c *initMysql) getCommand() []string {
	...
	return []string{"sh", "-c", "ulimit -s 1048576 && /docker-entrypoint.sh mysqld;if test -f /docker-entrypoint-initdb.d/plugin.sh; then /docker-entrypoint-initdb.d/plugin.sh; fi "}
}
