...
func TestGetInitMysqlCommand(t *testing.T) {
	assert.Equal(t, initMysqlCase.Command, []string{"sh", "-c", "ulimit -n 1048576 && /docker-entrypoint.sh mysqld;if test -f /docker-entrypoint-initdb.d/plugin.sh; then /docker-entrypoint-initdb.d/plugin.sh; fi "})
}
